...
| 字段建议 | field suggestion | 从评论中识别出的结构化细节（如"有靠背""免费"），整理成待确认字段推给条目发布者 |
...
| F11 | 评论细节字段建议 | 从可见评论中识别新细节 → 整理成待补充字段 → 推给条目发布者确认 → 确认结果自动写回条目 |
...
- **分级审核策略**：
  - 信用分 < 60 或历史通过评论 < 3 条的用户 → 评论先进入 `pending`，审核后可见；
  - 信用分 ≥ 60 的用户 → 先发后审，实时可见；
...
### 5.6b F11 评论细节字段建议（评论 → 待补充字段 → 作者确认 → 写回条目）

评论里常常藏着结构化的新细节（"这个长椅有靠背""直饮水是免费的，冬天会关"），但它们混在自然语言里，条目作者不一定看得到、也不一定会去改条目。本功能把这些细节**识别**出来、整理成**待补充字段**推给作者确认，作者一键采纳后**自动写回原始条目**。

**入口**：

- 条目详情页评论区上方（作者视角）出现"来自评论的补充建议"卡片；
- `/me` 个人空间"待我确认的补充"标签页；
- 通知中心铃铛。

**处理**：

1. **识别（规则引擎，非 LLM）**。一条评论变为 `visible` 时（先发后审的发表瞬间、或先审后发通过审核时），系统用该条目分类的**当前属性 Schema** 与内置中文词表逐条匹配，识别出评论者明确断言的属性取值。识别是纯函数规则匹配，不依赖任何外部模型或网络服务，任何机器上结果一致、可解释。
2. **只识别"补充"，不识别"纠正"**。只有当该属性在条目中**尚无值**（键不存在或为空）时才生成建议；条目已有值的字段即使评论表述不同也不生成（那属于"信息已过期"，走 F6 的过期上报与复核通道，而不是静默覆盖作者的判断）。
3. **可支持的字段类型**：布尔、枚举单选（string + enum）、整数（integer）、枚举多选（array + items.enum）。自由文本（无 enum 的 string）不做识别，避免把一句话硬塞进字段。词表未覆盖的字段跳过，绝不猜测。
4. **可追溯**。每条建议保存：来源评论、字段键与字段名、推断值与可读值文案、评论中的命中原句（evidence）、匹配到的关键词。
5. **去重与限额**：同一（条目、字段）只保留一条 `pending` 建议；新的命中原句追加到该建议的 evidence 中；一条评论每次识别最多产生 3 条建议。作者本人在自己条目下的评论不识别。
6. **推送给发布者**。识别出新建议后向条目作者发 `field_suggested` 通知（同一评论的多条建议合并为一条通知）。
7. **作者确认**。作者可**采纳**或**忽略**（可填理由）：
   - 采纳：把建议值合并写入 `spots.attributes`，并生成一个新的 `spot_revisions` 快照（`editor_id` 为作者，快照标注来源为评论建议）以保留完整可追溯链路；已发布条目不因此下线、不重新走审核——作者只是在补充自己记录的细节。写回时再次校验取值仍符合当前 Schema，不符合则返回 422 而不是写入脏数据。
   - 忽略：建议关闭，记录理由，不改动条目。
8. **状态机**：`pending → accepted / dismissed / expired`。来源评论被隐藏或删除时，其 `pending` 建议自动转 `expired`；`pending` 建议 30 天未处理由定时任务转 `expired`。每个状态都有出口，不会堆积。
9. **反馈与激励**：建议被采纳后通知来源评论者（`field_suggestion_accepted`），并为其增加少量信用分；被忽略时也通知评论者并附作者填写的理由（未填理由则给出通用文案）。

**结果**：作者在卡片上看到"有靠背 = 是（评论原文：'旁边这个长椅有靠背'）"，点采纳后条目"现场细节"立即多出这一项，评论者收到"你的补充被采纳"的通知。

**反馈**：无任何可识别细节的评论静默处理，不打扰作者；所有建议状态对评论者与作者双向可见。

**闭环出口**：`pending` 可采纳/忽略；评论失效自动过期；30 天超时自动过期；采纳即写回条目并产生修订快照；忽略/采纳/过期均通知或可查询，不存在只写不读的孤立数据。
...
| `field_suggested` | 自己的条目有来自评论的字段建议 | 条目详情页（建议卡片） |
| `field_suggestion_accepted` | 自己评论里的细节建议被作者采纳 | 条目详情页 |
| `field_suggestion_dismissed` | 自己评论里的细节建议被作者忽略 | 条目详情页 |
...
- **定时任务**（BullMQ 队列 + 调度）：
  ...
  - `expire_suggestions`：每天将超过 30 天未处理的字段建议置为 `expired`（并入 cleanup 周期执行）。
...
-- ============ 评论细节字段建议（F11） ============
-- 从可见评论中识别出的结构化细节，推给条目发布者确认；
-- 采纳后值写回 spots.attributes 并产生修订快照。
CREATE TABLE field_suggestions (
  ...
  -- 推断出的原始值（boolean/string/integer/string[]），采纳时按 Schema 再校验
  proposed_value JSONB NOT NULL,
  -- 可读值文案，如"是""免费""可容纳 4 人"
...
5. 字段建议采纳写回时，`proposed_value` 必须通过该分类**当前**属性 Schema 的类型与取值校验，否则拒绝写回（服务层强校验）；写回与修订快照生成在同一事务内。
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/spots/:uuid/suggestions` | 公开 | 某条目下的建议（仅 `accepted`）；作者额外看到全部状态 |
| GET | `/me/suggestions` | user | 待我确认 / 我提出的建议，`?scope=incoming`（默认）/ `outgoing` |
| POST | `/suggestions/:id/accept` | 条目作者 | 采纳：写回属性并生成修订快照 |
| POST | `/suggestions/:id/dismiss` | 条目作者 | 忽略（`reason` 可选，≤ 200 字） |

**识别触发时机**：评论创建后即为 `visible`（先发后审）时，或先审后发评论通过审核时，服务端自动识别，无独立"识别"接口。

**GET /me/suggestions?scope=incoming 响应片段**
  ...
  "items": [
    {
      "id": "10231",
      "status": "pending",
      "fieldKey": "has_backrest",
      "fieldLabel": "是否有靠背",
      "fieldType": "boolean",
      "valueLabel": "是",
      "evidence": ["旁边这个长椅有靠背，挺舒服的"],
      "createdAt": "2026-09-18T08:12:00Z",
      "comment": { "id": "8821", "body": "旁边这个长椅有靠背，挺舒服的" },
      "commenter": { "nickname": "路人甲" },
      "spot": { "uuid": "b1c2...", "title": "梧桐树下的长椅", "categoryCode": "bench" }
    }
  ],
...
服务端强制校验：只有建议所属条目的作者能采纳/忽略；`spot.status` 必须为 `published`；建议必须为 `pending` 且未过期；`proposed_value` 必须通过当前 Schema 校验。任一不满足返回 404/409/422 并给出 `SPOT_STATE_INVALID` 等错误码。采纳后同一评论若还有其他 `pending` 建议不受影响（逐条确认）。
...
| GET | `/admin/users` | admin | 用户列表（搜索/筛选） |
...
| `/me` | 个人空间（`?tab=suggestions` 定位到"待我确认的补充"） | 登录 |
...
| `FieldSuggestionPanel.vue` | 作者视角的字段建议卡片：字段、命中原句、采纳/忽略；采纳后刷新条目属性 |
...
│   ├── moderation/            # 内容过滤、重复检测、信用分、评论字段识别（fieldExtractor）
│   ├── queue/                 # BullMQ 队列定义
...
| 层级 | 工具 | 覆盖重点 |
|---|---|---|
| 单元测试 | Vitest | 坐标模糊化算法、新鲜度计算、权限判定、内容过滤、Schema 校验、评论字段识别词表与否定判断 |
...
| A16 | 评论细节字段建议闭环 | 用户评论"有靠背"→ 作者收到建议通知 → 采纳后条目属性多出该项并产生修订快照 → 评论者收到采纳通知；忽略/评论隐藏/30 天超时三条路径建议均正确关闭 |
...
- [ ] 可见评论中的已知字段（布尔/枚举/整数/多选）能被识别，并给出命中原句
- [ ] 条目已有值的字段不生成建议；自由文本字段不识别；词表未覆盖的字段不识别
- [ ] 同一（条目、字段）只有一条 pending 建议，新原句追加到 evidence
- [ ] 作者在自己条目下的评论不产生建议
- [ ] 作者收到合并通知，能在条目页与个人空间看到待确认建议
- [ ] 采纳后属性立即写回条目，并在修订历史中出现一个来源为评论建议的快照
- [ ] 采纳时再次校验 Schema，非法值无法写入
- [ ] 忽略可填理由；采纳与忽略都通知评论者
- [ ] 来源评论被隐藏/删除后建议自动过期；30 天未处理由定时任务过期
...
| Node.js 负责评论 | 评论服务、分级审核、内容过滤、举报联动、评论细节字段识别与作者确认写回（第 5.6、5.6b、10.7b 节） |
