...
- **评论细节提取与确认闭环**：公开的评论会经过一个离线规则引擎（`services/detail-extraction`），
  按分类 Schema 识别其中的结构化细节（布尔、枚举、数量、多选项），只提取**条目尚未填写**的字段，
  生成 `spot_suggestions` 推送给条目发布者：
  - 评论含 PII/敏感内容被拦截时不提取；先审后发的评论在审核通过后才提取，先发后审的评论发表即提取；
  - 每条建议带评论原句（证据）、字段名、建议值；同字段同值的重复建议累加 `support_count`，矛盾建议不重复建单；
  - 发布者在条目详情页或 `/me/suggestions` 看到待确认列表，**采纳**后系统校验取值仍符合当前 Schema，
    直接合并写入 `spots.attributes`（只补空字段，永不覆盖作者已填的值）；**不采纳**可附理由，通知评论作者；
  - 待确认建议 30 天未处理自动失效；提取/通知失败不影响评论主流程；
  - 作者本人评论自己的条目不产生建议；待确认建议仅作者与审核角色可见，未核实信息不外显。
...
| `detail_suggestion` | 评论中识别出待确认的细节字段 | 条目详情页 |
| `detail_accepted` | 自己评论里补充的细节被采纳 | 条目详情页 |
| `detail_rejected` | 自己评论里补充的细节未被采纳（含理由） | 条目详情页 |
...
CREATE TABLE spot_suggestions (
  id                BIGSERIAL PRIMARY KEY,
  spot_id           BIGINT NOT NULL REFERENCES spots(id) ON DELETE CASCADE,
  field_key         VARCHAR(48) NOT NULL,        -- 对应分类 Schema 的 properties key
  field_label       VARCHAR(48) NOT NULL,
  field_type        VARCHAR(16) NOT NULL,        -- boolean/string/integer/array
  proposed_value    JSONB NOT NULL,              -- 标量或字符串数组
  evidence          VARCHAR(120) NOT NULL,       -- 评论原句摘录
  status            suggestion_status NOT NULL DEFAULT 'pending',
  source_comment_id BIGINT NOT NULL REFERENCES comments(id) ON DELETE CASCADE,
  source_user_id    BIGINT NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
  support_count     INTEGER NOT NULL DEFAULT 1,  -- 同值补充人数
...
CREATE INDEX idx_suggestions_spot   ON spot_suggestions(spot_id, status, created_at DESC);
CREATE INDEX idx_suggestions_source ON spot_suggestions(source_user_id, status);
...
| POST | `/moderation/comments/:id/approve` | moderator | 通过（先审后发场景；通过时触发细节提取） |
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/me/suggestions` | user | 我收到的待确认补充（可按 status 过滤） |
| GET | `/me/suggestions/contributed` | user | 我在评论里贡献过的建议及处理结果 |
| GET | `/spots/:uuid/suggestions` | 作者/moderator | 该条目的待确认建议 |
| POST | `/suggestions/:id/accept` | 作者 | 采纳：校验通过后写回 `spot.attributes` |
| POST | `/suggestions/:id/reject` | 作者 | 不采纳（理由选填，通知评论作者） |
...
| `SuggestionPanel.vue` | 条目详情页"评论里的新细节，等你确认"卡片：采纳写回 / 不采纳 |
...
│   ├── reviews/               # 队列、领取、决策、申诉
│   ├── comments/              # 评论与回复
│   ├── suggestions/           # 评论细节建议：提取去重、发布者确认、采纳写回/驳回
│   ├── reports/               # 举报与处置
...
│   ├── detail-extraction/     # 评论→结构化字段的规则引擎（关键词、否定窗口、最长匹配）
│   ├── queue/                 # BullMQ 队列定义
...
**E2E-6 评论细节提取与确认写回**
条目留一个未填属性（如材质）→ 其他用户评论"椅子是金属的" → 评论公开后作者收到"待确认细节"通知 → `/me/suggestions` 与条目详情页出现带评论原句的建议卡片 → 作者点"采纳" → `spot.attributes` 立即写入 `material=metal`，无需重新审核 → 评论作者收到采纳通知；再用"像木头其实是塑料"的评论验证否定语义不被误提取，"不采纳"并附理由后评论作者收到驳回通知、条目不变。
...
| A14 | 通知全链路可达 | 触发全部通知类型，均能在通知中心看到并可跳转 |
| A15 | 无任何 TODO / mock / 假数据残留 | 全局搜索 `TODO`、`FIXME`、`mock` 等，仅允许出现在测试夹具与文档示例中 |
| A16 | 评论细节提取闭环 | 触发 E2E-6：建议只补空字段、作者确认后写回条目、驳回不改条目、30 天自动失效、非作者无权操作 |
...
- [ ] 确认与过期上报影响新鲜度
- [ ] 3 次过期上报后条目进入待复核
- [ ] 公开评论能提取出条目未填的结构化字段并生成待确认建议
- [ ] 建议采纳后自动写回条目属性，驳回不改条目，30 天未处理自动失效
- [ ] 建议只对条目作者可见，非作者的查看/操作均被拒绝
...
- [ ] 全部通知类型（审核 3 种、申诉、评论 2 种、举报、过期、细节建议 3 种）均能触发并展示
- [ ] 通知可跳转到对应位置
- [ ] 已读/全部已读可用
- [ ] 邮件开关生效
